...
class BTCPSocket:
    ...
    def __init__(self, window, timeout):
        ...
        self.sendingBufferEmpty = asyncio.Event()
    ...
    def create_data_segments(self, data: bytearray):
        ...
        while len(data) >= PAYLOAD_SIZE:
            ...
            #create a future for each segments' acknowledgment
            segments.append([self.create_data_segment(segmentpayload), asyncio.Event()])

        # If there's still some data left, put it in a segment
        if len(data) > 0:
            self._seqnum += 1
            segments.append([self.create_data_segment(data), asyncio.Event()])
    ...
    def send(self, data):
        ...
        for i in range(0, min(len(segments), self._rwindow)):
            if len(segments) > 0:
                self._lossy_layer.send_segment(segments[i][0].make())

                self.sentpackets += 1
                loop = asyncio.get_event_loop()
                loop.run_until_complete(asyncio.wait([self.awaitAckTimeout(segments[i][1], segments[i][0].seqnumber)]))
            else:
                break

    async def awaitAckTimeout(self, event, seqnum):
        await self.awaitAck(event, seqnum)

    async def awaitAck(self, event, seqnum):
        try:
    ...
    def recv_ACK(self, segment, rawSegment):
        ...
        if self.cksumOK(rawSegment) and segment.acknumber <= self._seqnum:
    ...
    def cksumOK(self, segment):
        return self.in_cksum(segment) == 0x0000
